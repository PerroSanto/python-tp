...
class  PinchaduraDeLata(threading.Thread):
    ...
    def pincharLata(self):
        ...
        if unaHeladera.hayLatas():
            unaHeladera.contenedorLatas.remove(0)
            self.impresionPinchadura(unaHeladera)

    def impresionPinchadura(self,unaHeladera):
        print(self.name,": se pincho una lata en",unaHeladera.name)
        self.separador()
...
#Aca teniamos pensado meter un segundo empleado para que se encargue de sacar las latas pinchadas, ya que intentamos 
#meter un monitor que notificaba cuando se pinchaba una lata pero el empleado se queda esperando la notificacion
#y no seguia cargado las heladeras.

#En un principio hicimos que los proveedores carguen las heladeras pero lo cambiamos porque leimos varias veces los
#puntos del ejercicio al leer esto: "Al bar llegan los proveedores y nos entregan cervezas al azar, tanto en cantidad como en tipo de envase. "
#Entonces supusimos que solo dejaban la mercaderia...
...
PinchaduraDeLata(name="Pinchadura").start()
